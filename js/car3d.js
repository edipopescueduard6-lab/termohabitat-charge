...
function mount(host, opts = {}) {
  ...
  function layout() {
    ...
    shadow.scale.set(size.x * 1.9, size.z * 1.3, 1);
    ...
    const a = new THREE.Vector3(charger.position.x - side * 0.12, 1.02, charger.position.z + 0.1);
  ...
  function frameCamera() {
    ...
    camera.userData.target = new THREE.Vector3(opts.mode === 'charging' ? size.x * 0.3 : 0, size.y * 0.4, opts.mode === 'charging' ? -size.z * 0.12 : 0);
  }
  loadModel(opts.model || 'suv').then(obj => {
    car = obj;
    /* piese de decor din fișierul original care nu țin de vehicul */
    (cfg.hide || []).forEach(n => { const o = car.getObjectByName(n); if (o) o.removeFromParent(); });
    /* pachete cu mai multe vehicule: păstrează caroseria aleasă și roțile aflate sub ea */
    if (cfg.pick) {
      const body = car.getObjectByName(cfg.pick);
      if (body) {
        ...
        holder.children.forEach(ch => { if (ch === body) return; const c = new THREE.Box3().setFromObject(ch).getCenter(new THREE.Vector3()); if (/wheel/i.test(ch.name) && bb.containsPoint(c)) keep.add(ch); });
    ...
    car.traverse(o => {
      ...
      mats.forEach((m, i) => {
        if (!m) return;
        if (m.envMapIntensity != null) m.envMapIntensity = cfg.env || 1.15;
        /* sticlă: transparentă, fumurie, cu reflexii */
        if (cfg.glassMats && cfg.glassMats.some(k => (m.name || '').toLowerCase().includes(k))) {
          const gm = new THREE.MeshPhysicalMaterial({ color: 0x0c1014, metalness: 0, roughness: 0.04, transparent: true, opacity: 0.55, envMapIntensity: 1.6, clearcoat: 1 });
          if (Array.isArray(o.material)) o.material[i] = gm; else o.material = gm; return;
        }
        /* vopsea personalizată pe materialele caroseriei */
        if (cfg.paint && cfg.paintMats && cfg.paintMats.some(k => (m.name || '').toLowerCase().includes(k))) {
          const pm = new THREE.MeshPhysicalMaterial({ color: new THREE.Color(cfg.paint), map: cfg.keepMap ? m.map : null, normalMap: m.normalMap || null, metalness: cfg.metal ?? 0.55, roughness: cfg.rough ?? 0.3, clearcoat: 1, clearcoatRoughness: 0.06, envMapIntensity: 1.3 });
          if (Array.isArray(o.material)) o.material[i] = pm; else o.material = pm;
        }
      });
    });
    const box = new THREE.Box3().setFromObject(car), s = new THREE.Vector3(); box.getSize(s);
    const len = cfg.length || 4.7, sc = len / Math.max(s.x, s.z);
    car.scale.setScalar(sc);
    car.rotation.y = cfg.rotY || 0;
  ...
  /* rotire cu degetul */
  let yaw = cfg.yaw != null ? cfg.yaw : (opts.mode === 'charging' ? 0.72 : 0.62), pitch = 0.2, dragging = false, lx = 0, vel = 0, lastInput = 0;
